...
exports.Bundle = class {
  constructor(bundler, config) {
    ...
    this.excludes = [];
    this.env = CLIOptions.getEnvironment();

    // this.prepend = (config.prepend || []).filter(x => bundler.itemIncludedInBuild(x));
    this.prepend = this._formatIncludeExclude(config.prepend || [], x => x).filter(x => bundler.itemIncludedInBuild(x));

    if (this.includes instanceof Array) {
      this.includes = this.includes.map(x => new SourceInclusion(this, x));
    } else {
      this.excludes = this._formatIncludeExclude(this.includes.exclude || [], ex => this.createMatcher(ex));
      this.includes = this._formatIncludeExclude(this.includes.include, x => new SourceInclusion(this, x));
    }
  ...
  _formatIncludeExclude(patterns, operation) {
    return patterns.map( pattern => {
      if (typeof pattern === 'string' || pattern instanceof String) {
        return operation(pattern);
      } else {
        return pattern.env === this.env ? operation(pattern.source) : null;
      } 
    }).filter(x => x);
  }
