...
exports.BundledSource = class {
  ...
  transform() {
    ...
    // Handle global deps
    const globalDeps = bundler.project.globalDependencies;
    const globalDepsCache = this._globalDepsCache;
        ...
        fileRead: function(defaultRead, id, filePath) {
          ...
          try {
            ...
            if (e.code === 'ENOENT') {
              ...
              if(globalDeps && globalDeps.length > 0) {

                // Check if module is not in global depepencies
                for(let i = 0; i < globalDeps.length; i++) {
                  const globalDepPath = globalDeps[i];

                  // Dep already load in cache ? No ? Let's load.
                  if(!(globalDepPath in globalDepsCache)) {
                    globalDepsCache[globalDepPath] = fs.readFileSync(globalDepPath).toString();
                  }
                  
                  const regexDefine = new RegExp("define\(['|\"]" + filePath.endsWith(".js") ? id : filePath + "['|\"],");
                                   
                  if(regexDefine.test(globalDepsCache[globalDepPath])) {
                    isOk = true;
                    // console.log(`${id} found in global dependencies.`)
                    break;
                  }
                }
              }
